...
def get_ndaq_tickers(mongo_client, FINANCIAL_PREP_API_KEY):
    ...
    def call_ndaq_100():
        ...
        try:
            ...
            symbols = [stock["symbol"] for stock in ndaq_stocks]
            dollar_volume = yf.download(symbols, period="1d")
            dollar_volume = dollar_volume["Close"] * dollar_volume["Volume"]
            dollar_volume = dollar_volume.sum(axis=0)

            # Sort the stocks by dollar volume
            ndaq_stocks = sorted(
                ndaq_stocks, key=lambda x: dollar_volume[x["symbol"]], reverse=True
            )
...
def get_latest_price(ticker):
    ...
    if x < 1 or x is None:
        try:
            ...
            x = ticker_yahoo.info.get("currentPrice")
            if x is None:
                data = ticker_yahoo.history()
                x = round(data["Close"].iloc[-1], 2)
            return x
        except Exception as e:
